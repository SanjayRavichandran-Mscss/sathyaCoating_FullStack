...
exports.addPettyCash = async (req, res) => {
  try {
    ...
    assignments.forEach((assignment, index) => {
      const { pd_id, site_id, assign_date, amount } = assignment;
    ...
    const insertedIds = [];
    for (const { pd_id, site_id, assign_date, amount } of assignments) {
      // Check for existing record with same pd_id, site_id, and assign_date
      const [existingRecord] = await db.query(
        'SELECT id FROM petty_cash WHERE pd_id = ? AND site_id = ? AND assign_date = ?',
        [pd_id, site_id, assign_date]
      );
      
      if (existingRecord.length > 0) {
        return res.status(400).json({
          status: 'error',
          message: `Petty cash already allotted for project ${pd_id}, site ${site_id} on ${assign_date}`
        });
      }

      const [site] = await db.query('SELECT site_id FROM site_details WHERE site_id = ? AND pd_id = ?', [site_id, pd_id]);
      if (site.length === 0) {
        return res.status(400).json({
          status: 'error',
      ...
      const [result] = await db.query(
        'INSERT INTO petty_cash (pd_id, site_id, assign_date, amount, created_at) VALUES (?, ?, ?, ?, NOW())',
        [pd_id, site_id, assign_date, amount]
      );
    ...
    if (error.code === 'ER_NO_REFERENCED_ROW_2') {
      return res.status(400).json({
        status: 'error',
        message: 'Invalid pd_id or site_id: referenced record does not exist',
      });
    }
    res.status(500).json({
      status: 'error',
...
exports.updatePettyCash = async (req, res) => {
  try {
    ...
    const [pettyCash] = await db.query('SELECT amount FROM petty_cash WHERE id = ?', [id]);
    ...
    // Update the amount
    const [result] = await db.query('UPDATE petty_cash SET amount = ?, updated_at = NOW() WHERE id = ?', [amount, id]);
    ...
    if (error.code === 'ER_NO_REFERENCED_ROW_2') {
      message = 'Invalid reference data in the database';
...
//       // Find the previous day's record for the same pd_id and site_id
//       const [previousRecord] = await db.query(`
//         SELECT pc.amount, COALESCE(SUM(see.amount), 0) as total_expenses
//         FROM petty_cash pc
//         LEFT JOIN siteincharge_exp_entry see ON pc.id = see.petty_cash_id
//         WHERE pc.pd_id = ? AND pc.site_id = ? AND pc.assign_date < ?
//         GROUP BY pc.id, pc.amount
//         ORDER BY pc.assign_date DESC
//         LIMIT 1
//       `, [pd_id, site_id, assign_date]);

//       let previous_remaining_amount = 0;
//       if (previousRecord.length > 0) {
//         previous_remaining_amount = previousRecord[0].amount - previousRecord[0].total_expenses;
//       }

//       // Calculate total amount: new amount + previous remaining amount
//       const total_amount = amount + previous_remaining_amount;

//       const [result] = await db.query(
//         'INSERT INTO petty_cash (pd_id, site_id, assign_date, amount, previous_remaining_amount, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
//         [pd_id, site_id, assign_date, total_amount, previous_remaining_amount]
//       );
//       insertedIds.push(result.insertId);
//     }
...
//     // Fetch current petty cash record
//     const [pettyCash] = await db.query('SELECT amount, previous_remaining_amount FROM petty_cash WHERE id = ?', [id]);
//     if (pettyCash.length === 0) {
//       return res.status(404).json({
//         status: 'error',
//         message: 'Petty cash record not found for the provided ID'
...
//     // Check existing expenses if trying to decrease amount
//     const currentPreviousRemaining = pettyCash[0].previous_remaining_amount || 0;
//     const currentTotalAmount = pettyCash[0].amount;
//     const newTotalAmount = amount + currentPreviousRemaining;
...
//       if (totalExpenses > newTotalAmount) {
//         return res.status(400).json({
//           status: 'error',
//           message: `Cannot reduce amount below ₹${totalExpenses.toFixed(2)} as this amount has already been expensed`
//         });
...
//     // Update the amount
//     const [result] = await db.query('UPDATE petty_cash SET amount = ?, updated_at = NOW() WHERE id = ?', [newTotalAmount, id]);
//     if (result.affectedRows === 0) {
//       return res.status(400).json({
//         status: 'error',
//         message: 'Failed to update petty cash amount: No changes applied'
