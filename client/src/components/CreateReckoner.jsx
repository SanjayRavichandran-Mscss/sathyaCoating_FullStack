...
const CreateReckoner = () => {
  ...
  const handleCategoryChange = (categoryIndex, e) => {
    const selectedCategory = categories.find(cat => cat.category_name === e.target.value);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/reckoner/reckoner', submissionData);
      ...
      {location.state && (
        <div className="p-4 mb-4">
          <div className="text-lg font-semibold text-blue-800">
            {siteName} <span className="text-gray-600">(PO: {poNumber})</span>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          {formData.categories.map((category, categoryIndex) => (
            <div key={categoryIndex} className={`border rounded-lg p-4 space-y-4 ${getRandomColor(categoryIndex)} border-2`}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {loading.categories && <p className="text-sm text-gray-500">Loading categories...</p>}
                </div>
              ...
              {category.categoryName && (
                <div className="space-y-6 mt-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-md font-medium text-gray-700">Subcategories</h3>
                    <button
                      type="button"
                  ...
                  {category.subcategories.map((subcategory, subcatIndex) => (
                    <div key={subcatIndex} className={`border rounded-lg p-4 space-y-4 ${getRandomColor(subcatIndex + 1)} border-2`}>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700">Subcategory Name</label>
                          <select
                            value={subcategory.subcategoryName}
                            onChange={(e) => handleSubcategoryNameChange(categoryIndex, subcatIndex, e.target.value)}
                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border"
                            required
                          ...
                          {loading.subcategories && <p className="text-sm text-gray-500">Loading subcategories...</p>}
                        </div>
                        <div className="flex items-end justify-end">
                      ...
                      {subcategory.subcategoryName && (
                        <div className="mt-4">
                          <h4 className="text-md font-medium mb-2 text-gray-700">Items</h4>
                          <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                              <thead className={`${getRandomColor(subcatIndex + 2)}`}>
                                <tr>
                                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Item No</th>
                                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Item Description</th>
                                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">PO Quantity</th>
                                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Unit of Measure</th>
                                  ...
                                  <tr key={itemIndex} className={itemIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                      <select
                                        ...
                                        onChange={(e) => handleItemDescriptionChange(categoryIndex, subcatIndex, itemIndex, e.target.value)}
                                      ...
                                      >
                                        ...
                                        {workItems.map(item => (
                                          <option key={item.item_id} value={item.item_description}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                      <input
                                        ...
                                        onChange={(e) => handleItemChange(categoryIndex, subcatIndex, itemIndex, e)}
                                        className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border"
                                        required
                                        min="0"
                                        step="0.01"
                          ...
                          <div className="mt-2">
                            <button
                              type="button"
                              onClick={() => addItemRow(categoryIndex, subcatIndex)}
                              className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                            >
                              Add Item
                            </button>
